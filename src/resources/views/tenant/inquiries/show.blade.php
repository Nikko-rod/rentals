...
<div class="max-w-7xl mx-auto space-y-4">
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden">
        ...
        <div class="p-4">
            <div class="flex flex-wrap gap-4">
                ...
                <div class="flex items-center gap-2 bg-blue-50 text-blue-800 px-3 py-1.5 rounded-lg border border-blue-100">
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden">
        <div class="p-4 border-b border-gray-100 flex justify-between items-center">
            ...
            <span class="text-sm text-gray-500">{{ $messages->count() ?? 0 }}/10 messages</span>
        </div>

        <div class="p-4 max-h-[500px] overflow-y-auto space-y-4 bg-gray-50" id="messageContainer">
    @foreach($messages as $message)
        <div class="flex {{ $message->is_landlord ? 'justify-start' : 'justify-end' }}">
            <div class="max-w-[400px] space-y-1"> <!-- Changed to fixed pixel width -->
                <div class="px-4 py-2 rounded-t-xl break-words whitespace-pre-wrap overflow-hidden text-sm
                    {{ $message->is_landlord ? 'bg-white text-gray-800 rounded-br-xl rounded-bl-sm' : 'bg-green-600 text-white rounded-bl-xl rounded-br-sm' }} 
                    shadow-sm">
                    {!! nl2br(e($message->message)) !!}
                </div>
                <div class="text-xs text-gray-500 {{ $message->is_landlord ? 'text-left' : 'text-right' }}">
    ...
    @endforeach
        ...
        <div class="p-4 border-t border-gray-100 bg-white">
            ...
            @if($messageCount >= 10)
                <div class="text-center text-yellow-600 bg-yellow-50 p-3 rounded-lg">
                    <i class="fas fa-exclamation-circle mr-2"></i>
                    Maximum messages reached (10/10)
                </div>
            @elseif(!$inquiry->canReply())
                <div class="text-center text-blue-600 bg-blue-50 p-3 rounded-lg">
                    <i class="fas fa-clock mr-2"></i>
                    Wait for the Landlord's response
                </div>
            @else
            <form action="{{ route('tenant.inquiries.reply', $inquiry) }}" method="POST" class="space-y-3">
    @csrf
    <div class="relative">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Handle Enter key for new lines (Shift+Enter)
